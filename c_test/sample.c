...
int main(int argc,char **argv){
	...
	int inst_cre_result = vkCreateInstance(&inst_cre_info,NULL,&inst);
	if (inst_cre_result != 0) {
		printf("Instance creation failed, code: %d\n", inst_cre_result);
	};
	...
	FILE *fp_vert=NULL,
		 *fp_frag=NULL;
	fp_vert=fopen("shaders/v.spv","rb+");
	fp_frag=fopen("shaders/f.spv","rb+");
